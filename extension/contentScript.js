(() => {
  ...
  function ensureSidebarItem() {
    ...
    const subsAnchor = guide.querySelector('ytd-guide-entry-renderer a[href*="/feed/subscriptions"], ytd-guide-entry-renderer a[title="Subscriptions"]');
  ...
  async function loadHub() {
    ...
    try {
      ...
      list.innerHTML = vids.map(v => `
        <a href="https://www.youtube.com/watch?v=${encodeURIComponent(v.youtube_video_id)}" style="text-decoration:none;color:inherit;border:1px solid #333;border-radius:12px;overflow:hidden;display:block;background:#0f0f0f;">
          <div style="position:relative;background:#000;aspect-ratio:16/9;">
            <img src="${v.thumbnail_url ? v.thumbnail_url : `https://i.ytimg.com/vi/${encodeURIComponent(v.youtube_video_id)}/hqdefault.jpg`}" style="width:100%;height:100%;object-fit:cover;display:block;"/>
            <div style="position:absolute;right:8px;bottom:8px;background:rgba(0,0,0,.7);padding:2px 6px;border-radius:6px;font-size:12px;">${v.comment_count || 0} comments</div>
          </div>
          <div style="padding:10px 12px;">
            <div style="font-weight:600;line-height:1.4;display:-webkit-box;-webkit-line-clamp:2;overflow:hidden;-webkit-box-orient:vertical;">${escapeHtml(v.title || v.youtube_video_id)}</div>
          </div>
        </a>
      `).join('');
    } catch (e) {
  ...
  function renderAuthArea() {
    ...
    if (!authArea) return;
    if (authToken) {
      const avatar = currentUser?.picture ? `<img src="${currentUser.picture}" style="width:22px;height:22px;border-radius:50%;object-fit:cover;"/>` : '';
    ...
    function itemHtml(c, level) {
      ...
      return `
        <div class="wec-item ${indentClass}" data-id="${c.id}">
          <div class="wec-row">
            <div>
              ${c.user?.picture ? `<img class="wec-avatar" src="${c.user.picture}"/>` : `<div class=\"wec-avatar\"></div>`}
            </div>
            <div>
  ...
  function startLoginFlow() {
    ...
    const popup = window.open(`${backendBase}/auth/google/start`, 'wecomment_login', `width=${w},height=${h},left=${x},top=${y}`);
    if (!popup) return;
  }
  ...
  async function fetchComments(videoId) {
    try {
      ...
      const res = await fetch(`${backendBase}/api/videos/${encodeURIComponent(videoId)}/comments?sort=${encodeURIComponent(sortMode)}`, { headers });
      const data = await res.json();
  ...
  function injectUI(anchorEl) {
    ...
    if (currentVideoId) {
      ...
      fetch(`${backendBase}/api/videos/${encodeURIComponent(currentVideoId)}?title=${encodeURIComponent(title)}`).catch(()=>{});
    }
        ...
        if (act === 'vote') {
          ...
          try {
            ...
            if (res.ok) {
              ...
              if (data.voted) btn.classList.add('is-active'); else btn.classList.remove('is-active');
            } else if (res.status === 401) {
              alert('Session expired. Please sign in again.');
              authToken = null;
              await chrome.storage.local.remove('wecomment_token');
